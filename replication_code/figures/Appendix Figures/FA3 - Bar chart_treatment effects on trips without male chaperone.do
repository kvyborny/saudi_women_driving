...
Purpose: 		APPENDIX FIGURE - TREATMENT FX ON TRIPS WITHOUT CHAPERONE


Table footnotes: This figure shows the results of a series of estimates of equation 1 in which the
outcome variables are mutually exclusive and exhaustive indicators for the frequency of
travel. Each control group bar shows the control group mean, while the treatment bar
shows the sum of the control group mean and the ITT treatment effect β1. Regressions
include individual and household controls: age (above median dummy), education level
(less than a highschool degree), marital status (indicators for married, never-married, and
widowed), household size (number of members), number of cars owned (indicators for one
car and for more than one car), an indicator for baseline labor force participation, and strata
fixed effects. SEs are clustered at household level. We replace missing control values with 0
