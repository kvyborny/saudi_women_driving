...
unemployment are similar if we redefine unemployed to include only those who applied for at least one job in the previous
month. In Panels B and C we re-estimate our results using survey weights to map to population estimates of education
according to age group (Panel B), and labor force participation (Panel C). We generate these weights using administrative data
from GASTAT (2017) and GASTAT (2018b), the latter is reported in Table A2. We use LFP, age, and education measured
in our sample at baseline. Variations in sample size are due to drop-off from telephone survey; order of survey modules was
randomized. All estimates include individual and household controls: age (above median dummy), education level (less than a
highschool degree), marital status (indicators for married, never-married, and widowed), household size (number of members),
number of cars owned (indicators for one car and for more than one car), an indicator for baseline labor force participation,
and randomization cohort fixed effects. SEs are clustered at household level. We impute for missing control values and include
...
* Set global for table outcomes
	global 	labor employed_w3  unempl_jobsearch_w3 not_in_LF_w3 ///
			empl_jobsearch_w3  
...
* Run models
	...
	* (1) WEIGHTED
	...
	global 	labor_wt`i' employed_w3_wt`i'  unempl_jobsearch_w3_wt`i' not_in_LF_w3_wt`i' ///
		...
		* Write to latex			 
		...
		"$output_rct/robustness/LFP_cohortPAP_`weight'_altunemp_`c(current_date)'.tex", ///
	...
	* (2) UNWEIGHTED PANEL FOR REFERENCE
	foreach var in $labor {	
		...
		* Write to latex			 
		esttab $labor using ///
		"$output_rct/LFP_cohortPAP_unweighted_altunemp_`c(current_date)'.tex", ///
		label se nonotes scalars("cmean Control mean" "b_cmean $\beta$/control mean"  "pval P-value $\beta = 0$") ///
		nobaselevels keep(treatment) nogaps  b(%9.3f) se(%9.3f) ///
		star(* 0.1 ** 0.05 *** 0.01) ///	
		mtitles( "\shortstack{Employed}" ///
